...
public class CheckPwdUtils {
    ...
    public boolean checkContainSpecialChar(String password) {
        char[] chPass = password.toCharArray();
        for (char pass : chPass) {
            if (config.getSpecialCharSet().indexOf(pass) != -1) {
                return true;
            }
        }
        return false;
     ...
     */
    public boolean checkLateralKeyboardSite(String password) {
        ...
        for (int i = 0; i + limit_num <= n; i++) {
            ...
            for (int j = 0; j < arrLen; j++) {
                ...
                //检测包含字母(区分大小写)
                if (config.getCheckContainUpperLowerCase()) {
                    //考虑 大写键盘匹配的情况
     ...
     */
    public boolean checkKeyboardSlantSite(String password) {
        String t_password = password;
        t_password = t_password.toLowerCase();
        int n = t_password.length();
        /*
     ...
     */
    public boolean checkSequentialChars(String password) {
        String t_password = password;
        boolean flag = false;
     ...
     */
    public boolean checkSequentialSameChars(String password) {
    ...
    public boolean EvalPWD(String password) {
        ...
        /*
          检测长度
         */
        if (config.getCheckPasswordLength()) {
        ...
        /*
          检测包含数字
         */
        if (config.getCheckContainDigit()) {
        ...
        if (config.getCheckContainUpperLowerCase()) {
            //检测包含小写字母
            if (config.getCheckContainLowerCase()) {
            ...
            //检测包含大写字母
            if (config.getCheckContainUpperCase()) {
        ...
        /*
          检测包含特殊符号
         */
        if (config.getCheckContainSpecialChar()) {
        ...
        检测包含字符类型数量
         */
        if (config.getCheckContainType() && i < config.getCheckContainTypeNum()) {
        ...
        /*
          检测键盘横向连续
         */
        if (config.getCheckHorizontalKeySequential()) {
        ...
        /*
          检测键盘斜向连续
         */
        if (config.getCheckSlopeKeySequential()) {
        ...
        /*
          检测逻辑位置连续
         */
        if (config.getCheckLogicSequential()) {
        ...
        /*
          检测相邻字符是否相同
         */
        if (config.getCheckSequentialCharSame()) {
