    ...
    protected SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http.csrf(AbstractHttpConfigurer::disable)
                ...
                .authorizeHttpRequests(authz -> authz
                        ...
                        .permitAll()
                        // 指定的匿名接口
                        .antMatchers(ANONYMOUS_URL)
                        .permitAll()
                        // 限制get请求匿名接口
                        .antMatchers(HttpMethod.GET, GET_ANONYMOUS_URL)
                        .permitAll()
                        // 限制post请求匿名接口
                        .antMatchers(HttpMethod.POST, POST_ANONYMOUS_URL)
                        .permitAll()
                        .anyRequest()
                        .authenticated())
        ...
        for (Map.Entry<RequestMappingInfo, HandlerMethod> infoEntry : handlerMethods.entrySet()) {
            ...
            if (annotation != null) {
                ...
                if (infoEntry.getKey().getPathPatternsCondition() != null)
                    allAnonymousAccess.addAll(infoEntry.getKey().getPathPatternsCondition().getPatterns().stream().map(PathPattern::getPatternString).collect(Collectors.toSet()));
            }
        }
